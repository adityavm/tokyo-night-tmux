...
#+--- Bars LEFT ---+
# Session name
tmux set -g status-left "#{?client_prefix,#[fg=${THEME[black]}]#[bg=${THEME[bred]}]#[bold],#[fg=${THEME[white]}]#[bg=${THEME[black]}]#[bold]} #{?client_prefix,󰠠 ,#[dim]󰤂 }#[bold,nodim]#S "

#+--- Windows ---+
# Focus
tmux set -g window-status-current-format "$RESET#[fg=${THEME[green]},bg=${THEME[bblack]}] #{?#{==:#{pane_current_command},ssh},󰣀 , }#[fg=${THEME[foreground]},bold,nodim]$window_number#W#[nobold]#{?window_zoomed_flag, $zoom_number, $custom_pane}#{?window_last_flag,,}#{?pane_synchronized,󰿨 ,}"
# Unfocused
tmux set -g window-status-format "$RESET#[fg=${THEME[foreground]}] #{?#{==:#{pane_current_command},ssh},󰣀 , }${RESET}$window_number#W#[nobold,dim]#{?window_zoomed_flag, $zoom_number, $custom_pane}#[fg=${THEME[yellow]}]#{?window_last_flag,󰁯 ,}#{?pane_synchronized,󰿨 ,}"
